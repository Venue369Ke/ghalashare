class GuestReviewsController < ApplicationController
  ...
  def create
    # Step 1: Check if the reservation exist (warehouse_id, host_id, host_id)
    ...
    @reservation = Reservation.where(
                    ...
                    warehouse_id: guest_review_params[:warehouse_id]
                   ).first

    if !@reservation.nil? && @reservation.warehouse.user.id == guest_review_params[:host_id].to_i
  ...
  private
    def guest_review_params
      params.require(:guest_review).permit(:comment, :star, :warehouse_id, :reservation_id, :host_id)
    end
end
