...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { config_item: @config.config_item, warehouse_id: @config.warehouse_id } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { config_item: @config.config_item, warehouse_id: @config.warehouse_id } }
